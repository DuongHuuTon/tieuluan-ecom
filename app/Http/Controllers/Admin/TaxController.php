...
class TaxController extends Controller
{
    ...
    public function manage_tax_process(Request $request)
    {
        ...
        if ($request->post('id') > 0) {
            ...
            $msg = "Cập nhật thuế thành công!";
        } else {
            $model = new Tax();
            $msg = "Thêm thuế thành công!";
        }
    ...
    public function delete(Request $request, $id)
    {
        ...
        $model->delete();
        $request->session()->flash('message', 'Thuế đã được xóa!');
    ...
    public function status(Request $request, $status, $id)
    {
        ...
        $model->save();
        $request->session()->flash('message', 'Đã cập nhật trạng thái thuế!');
